...
def inspect(artifact, manifest, legacy=False, max_glibc=None):
    ...
    if ceiling and glibc and glibc[-1] > ceiling:
    ...
    return {'artifact': str(artifact), 'passed': not errors, 'errors': errors,
            ...
            'glibc_ceiling': max_glibc, 'kernel_compatibility': 'not established by ELF inspection'}
...
def main():
    ...
    a = p.parse_args()
    try:
        report = inspect(a.artifact.resolve(strict=True), a.manifest.resolve(strict=True), a.legacy_fork, a.max_glibc)
