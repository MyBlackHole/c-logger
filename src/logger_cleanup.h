...
#define __cleanup(function) __attribute__((cleanup(function)))
...
#define DEFINE_FREE(_name, _type, _release)                                \
	static __always_inline void __free_##_name(void *__slot)            \
	{                                                                     \
...
#define DEFINE_CLASS(_name, _type, _exit, _init, ...)                        \
	...
	static __always_inline void class_##_name##_destructor(_type *__slot)  \
	{                                                                     \
		_type _T = *__slot;                                             \
		_exit;                                                           \
	}                                                                     \
	static __always_inline _type class_##_name##_constructor(__VA_ARGS__)  \
	{                                                                     \
		return (_init);                                                  \
	}
...
#define EXTEND_CLASS_COND(_name, _ext, _cond, _init, ...)                   \
	...
	static __always_inline class_##_name##_t                                \
...
static __always_inline int __cleanup_normalize_lock_error(int rc)
{
	if (rc > 0)
		return -rc;
	if (rc < 0)
...
static __always_inline void __cleanup_release_free(void *ptr)
{
	int saved = errno;
	free(ptr);
	errno = saved;
...
static __always_inline void __cleanup_release_fd(int fd)
{
	int saved = errno;
	(void)close(fd);
	errno = saved;
...
static __always_inline void __cleanup_release_file(FILE *file)
{
	int saved = errno;
	(void)fclose(file);
	errno = saved;
