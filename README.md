...
cmake --install build --prefix /opt/logger/0.9.3
```
...
共享产物 `liblogger.so.0.9.3` / SONAME `liblogger.so.0`；静态版本使用独立 build/prefix。
...
- [Xmake 并行构建与迁移门禁](docs/XMAKE_PARITY.md)
...
- [Queue self-paced wakeup 验证](validation/QUEUE_WAKEUP.md)
...
- [0.9.3 发布说明](docs/RELEASE_NOTES_0.9.3.md)
