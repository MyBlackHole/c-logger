...
- 软件版本 `0.9.2`；候选 ABI major `0`；Linux SONAME `liblogger.so.0`。0.9.2 未新增/删除 public C symbol，继续使用 `LOGGER_0.9` 版本节点。
- 实体文件 `liblogger.so.0.9.2`，构建/安装系统生成 `liblogger.so.0`、`liblogger.so` 相对软链接。
...
- `find_package(Logger 0.9.2 EXACT CONFIG REQUIRED)`：pre-1.0 使用 ExactVersion，
...
cmake -S . -B build-shared -DCMAKE_BUILD_TYPE=Release \
  ...
  -DCMAKE_INSTALL_PREFIX=/opt/logger/0.9.2
...
cmake -S . -B build-static -DCMAKE_BUILD_TYPE=Release \
  ...
  -DCMAKE_INSTALL_PREFIX=/opt/logger/0.9.2-static
...
<prefix>/lib/liblogger.so.0.9.2（或 liblogger.a）
...
```cmake
find_package(Logger 0.9.2 EXACT CONFIG REQUIRED) # 可指定 COMPONENTS shared/static/legacy_fork
...
```sh
PKG_CONFIG_PATH=/opt/logger/0.9.2/lib/pkgconfig \
