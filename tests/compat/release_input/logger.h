...
extern "C" {
#endif
typedef enum { LOGGER_TRACE, LOGGER_DEBUG, LOGGER_INFO, LOGGER_WARN, LOGGER_ERROR, LOGGER_FATAL, LOGGER_OFF } logger_level_t;
typedef enum {
...
typedef enum { LOGGER_OUT_STDERR=1u<<0, LOGGER_OUT_FILE=1u<<1, LOGGER_OUT_SYSLOG=1u<<2 } logger_output_t;
typedef enum {
...
#define LOGGER_SOURCE() ((logger_source_t){.module=LOG_MODULE,.file=__FILE__,.func=__func__,.line=__LINE__})
...
#define LOGGER_SYSLOG_DEFAULT_CONFIG() ((logger_syslog_config_t){ \
    .path=NULL,.facility=LOGGER_SYSLOG_FACILITY_USER,.reconnect_interval_ms=1000u, \
...
typedef struct {
    ...
    uint32_t version;
    logger_level_t level; logger_detail_t detail; unsigned outputs; const char *file_path;
    logger_rotation_config_t rotation; unsigned file_mode; int async_mode; size_t queue_capacity;
    logger_level_t flush_level; int include_pid, include_tid, include_source; const char *ident;
...
void logger_log(logger_t *, logger_level_t, const char *, const char *, int, const char *, const char *, ...)
#if defined(__GNUC__) || defined(__clang__)
...
int logger_get_syslog_metrics(logger_t *, logger_syslog_metrics_t *);
...
#if defined(LOGGER_ENABLE_LEGACY_FORK_HELPER) && LOGGER_ENABLE_LEGACY_FORK_HELPER
...
void logger_global_write(logger_level_t,const char *,const char *,int,const char *,const char *,...)
#if defined(__GNUC__) || defined(__clang__)
...
#endif
#define LOG_TRACE(...) logger_global_write(LOGGER_TRACE,LOG_MODULE,__FILE__,__LINE__,__func__,__VA_ARGS__)
#define LOG_DEBUG(...) logger_global_write(LOGGER_DEBUG,LOG_MODULE,__FILE__,__LINE__,__func__,__VA_ARGS__)
#define LOG_INFO(...)  logger_global_write(LOGGER_INFO, LOG_MODULE,__FILE__,__LINE__,__func__,__VA_ARGS__)
#define LOG_WARN(...)  logger_global_write(LOGGER_WARN, LOG_MODULE,__FILE__,__LINE__,__func__,__VA_ARGS__)
#define LOG_ERROR(...) logger_global_write(LOGGER_ERROR,LOG_MODULE,__FILE__,__LINE__,__func__,__VA_ARGS__)
#define LOG_FATAL(...) logger_global_write(LOGGER_FATAL,LOG_MODULE,__FILE__,__LINE__,__func__,__VA_ARGS__)
#define LOGGER_LOG(instance,level,module,...) \
    logger_log((instance),(level),(module),__FILE__,__LINE__,__func__,__VA_ARGS__)
#define LOGGER_TRACE(instance,module,...) LOGGER_LOG((instance),LOGGER_TRACE,(module),__VA_ARGS__)
#define LOGGER_DEBUG(instance,module,...) LOGGER_LOG((instance),LOGGER_DEBUG,(module),__VA_ARGS__)
#define LOGGER_INFO(instance,module,...)  LOGGER_LOG((instance),LOGGER_INFO, (module),__VA_ARGS__)
#define LOGGER_WARN(instance,module,...)  LOGGER_LOG((instance),LOGGER_WARN, (module),__VA_ARGS__)
#define LOGGER_ERROR(instance,module,...) LOGGER_LOG((instance),LOGGER_ERROR,(module),__VA_ARGS__)
#define LOGGER_FATAL(instance,module,...) LOGGER_LOG((instance),LOGGER_FATAL,(module),__VA_ARGS__)
#define LOGGER_DEFAULT_CONFIG() ((logger_config_t){.struct_size=sizeof(logger_config_t),.version=LOGGER_CONFIG_VERSION,.level=LOGGER_INFO,.detail=LOGGER_DETAIL_VERBOSE,.outputs=LOGGER_OUT_STDERR,.file_path=NULL,.rotation={.mode=LOGGER_ROTATE_SIZE_DAILY,.max_file_size=100u*1024u*1024u,.retention_days=30},.file_mode=0640,.async_mode=1,.queue_capacity=8192,.flush_level=LOGGER_ERROR,.include_pid=1,.include_tid=1,.include_source=1,.ident="app",.overflow={LOGGER_OVERFLOW_DROP,LOGGER_OVERFLOW_DROP,LOGGER_OVERFLOW_DROP,LOGGER_OVERFLOW_DROP,LOGGER_OVERFLOW_SYNC,LOGGER_OVERFLOW_SYNC},.syslog=LOGGER_SYSLOG_DEFAULT_CONFIG()})
#ifdef __cplusplus
}
#endif
#endif
