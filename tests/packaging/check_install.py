...
try:
    ...
    assert run(['pkg-config','--modversion','logger'],env=pe).strip()=='0.9.0'
    ...
    for version,component,success in [('0.9.0',a.kind,True),('0.9.1',a.kind,False),
        ('1.0.0',a.kind,False),('0.9.0','static' if a.kind=='shared' else 'shared',False),
        ('0.9.0','invented',False),('0.9.0','legacy_fork',a.legacy)]:
    ...
    if a.kind=='shared':
        ...
        assert dso.resolve().name=='liblogger.so.0.9.0'
